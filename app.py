...
if st.button("Predict"):
    ...
    predicted_proba = model.predict_proba(feature_values[0].reshape(1, -1))

    

# 假设 predicted_class 和 predicted_proba 分别是预测的类别和概率数组
for i in range(4):  # 假设有四个标签
    label_name = ""
    if i == 0:
        label_name = "Heart Failure"
    elif i == 1:
        label_name = "Myocardial Infarction"
    elif i == 2:
        label_name = "Stroke"
    elif i == 3:
        label_name = "Death"
    # 提取概率
    probability = predicted_proba[i][0][1]  # 假设您想显示的是每个标签的第一个类别的概率
