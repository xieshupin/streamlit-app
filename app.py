...
def predict_proba_for_label(data, model):
    """
    包装每个分类器的 predict_proba 函数，使其与 LIME 的要求一致。
    LIME 要求输入一个函数，该函数返回一个概率分布。
    """
    return model.predict_proba(data)  # 返回概率分布
...
if st.button("Predict"):
    ...
    feature_values_2d = feature_values[0].reshape(1, -1)
    training_data = np.tile(feature_values, (100, 1))  # 生成代表性数据集
    class_names = ["Heart Failure", "Myocardial Infarction", "Stroke", "Death"]
    explainer = LimeTabularExplainer(training_data, mode="classification",feature_names=feature_names)
        
    class_names = ["Heart Failure", "Myocardial Infarction", "Stroke", "Death"]
    for i, label in enumerate(class_names):
        st.write(f"### Explanation for '{label}'")
        
        # 包装当前分类器的 predict_proba
        predict_proba_fn = lambda x: predict_proba_for_label(x, classifiers[i])
        exp = explainer.explain_instance(feature_values_2d[0], predict_fn=predict_proba_fn)
        
        # 将 LIME 解释结果转为 matplotlib 图形并显示
        fig = exp.as_pyplot_figure()  # LIME 返回的解释结果转换为 matplotlib 图形
