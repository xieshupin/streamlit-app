...
if st.button("Predict"):
    ...
    predicted_proba = model.predict_proba(feature_values.iloc[0].values.reshape(1, -1))

    # 显示预测结果
    st.subheader("Prediction Results:")
    st.write("Risk Prediction:")
    st.write(f"- Heart Failure: {'Yes' if predicted_class[0]== 1 else 'No'} (Probability: {predicted_proba[0]:.2f})")
    st.write(f"- Myocardial Infarction: {'Yes' if predicted_class[1] == 1 else 'No'} (Probability: {predicted_proba[1]:.2f})")
    st.write(f"- Stroke: {'Yes' if predicted_class[2] == 1 else 'No'} (Probability: {predicted_proba[2]:.2f})")
    st.write(f"- Death: {'Yes' if predicted_class[3] == 1 else 'No'} (Probability: {predicted_proba[3]:.2f})")
