...
X_train = pd.read_pickle("data.pkl")  # 真实训练数据
# 英文特征和标签
...
if st.button("Predict"):
    ...
    for i, classifier in enumerate(classifiers):
        exp = explainer.explain_instance(feature_values[0], classifier.predict_proba)  # 解释当前分类器的预测
        lime_explanations.append(exp)  # 保存解释

        # 可视化 LIME 解释
        fig = exp.as_pyplot_figure()  # 将 LIME 解释结果转换为 matplotlib 图形
